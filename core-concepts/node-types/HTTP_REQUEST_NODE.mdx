...
*   **URL**
    ...
    * Variable interpolation using `{{variableName}}` syntax from [Orka Expression Language (OEL)](/core-concepts/OEL)
...
*   **Body**
    ...
    * Variable interpolation: Use `{{variableName}}` syntax from [OEL](/core-concepts/OEL) to include workflow variables
...
> You can use `$` to map the whole response body to the execution variables. Response mapping uses [OEL](/core-concepts/OEL) to extract and transform data from API responses.
